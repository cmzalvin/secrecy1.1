...
var instructor = new Ext.data.Record.create([ {
	name : 'id'
}, {
	name : 'name'
}, {
...
var college_instructorphotofields = [ {
	columnWidth : 1,
	layout : 'form',
	labelWidth : 0,
	items : [ {
...
// 高校用户
var college_instructorphotoform = new Ext.form.FormPanel({
	labelAlign : 'left',
	width : 170,
	height : 210,
	fileUpload : false,
	autoHeight : true,
	region : 'east',
	items : [ college_instructorphotofields ],
	buttonAlign : 'center'
});
...
var college_instructorform = new Ext.Panel({
	...
	layout : 'border',
	buttonAlign : 'center',
	items : [ college_instructorphotoform, college_instructorbasicform ],
	buttons : [
			{
				text : '保存',
				handler : function() {
					...
					if (college_instructorbasicform.getForm().isValid()) {
						...
						enableOfInstructorbasicform();
						instructor = college_instructorbasicform.getForm()
								.getValues();
						// stunum = Ext.getCmp("studentnum").getValue();
						UserController.updateInstructorAllInfo(
	...
	items : [ {
		...
		items : [ {
			...
			items : [ {
				xtype : 'textfield',
				fieldLabel : '辅导员姓名',
				name : 'and$instructor-name$like$%value%',
				anchor : '96%'
			} ]
		}, {
			columnWidth : .5,
			...
			items : [ {
				xtype : 'textfield',
				fieldLabel : '辅导员工号',
				name : 'and$instructor-instructor$=$value',
				anchor : '96%'
			} ]
		}, ]
	} ],
...
/*
 * var instructor_searchWin = new Ext.Window({ title: '查询条件', width: 500,
 * autoHeight:true, minWidth: 300, minHeight: 200, closeAction : 'hide', layout:
 * 'fit', plain:true, buttonAlign:'center', items: [instructor_searchform],
 * modal: true, shadow: true, buttons:[{ text : '查询', handler : function() {
 * instructor_search(); instructor_searchWin.hide(); } }, { text : '清空', handler :
 * function() { instructor_searchform.getForm().reset();
 * instructor_currentSearchFilter = ""; } }, { text : '关闭', handler : function() {
 * instructor_searchWin.hide(); } }] });
 */
			...
			'-',
			{
				...
				handler : function() {
					...
					college_instructorbasicform.getForm().loadRecord(
							emptyinstructor);
					instructorInfoWinInit("新增辅导员信息").show();
				}
			},
			'-',
			{
			...
			'-',
			{
				...
				handler : function() {
					if (instructorgrid.getSelectionModel().hasSelection()) {
						Ext.MessageBox.confirm('提示', '你确定要删除选中的辅导员?', function(
								button) {
							if (button == 'yes') {
	...
	listeners : {
		rowdblclick : function(grid) {
			...
			if (grid.getSelectionModel().hasSelection()) {
				instructorInfoWinInit("编辑辅导员信息").show();
...
/*
 * var importinstructorpicwin;
 * 
 * var importinstructorpicpanel = new Ext.form.FormPanel({ frame:true ,items:{
 * xtype:'awesomeuploader' ,gridHeight:280 ,height:350
 * ,awesomeUploaderRoot:'/InstructorHelpStd/resources/awesomeuploader/'
 * ,listeners:{ scope:this ,fileupload:function(uploader, success, result){
 * if(success){ Ext.Msg.alert('File Uploaded!','A file has been uploaded!'); } } } }
 * });
 */

/*
 * function studentPicImport(){ if(!importstudentpicwin){ importstudentpicwin =
 * new Ext.Window({ title:'照片导入' ,closeAction:'hide' ,width:520 ,height:350
 * ,layout : 'fit' ,items:[importstudentpicpanel] }); } return
 * importstudentpicwin; }
 */

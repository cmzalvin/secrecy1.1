	...
	public boolean updateInstructorAllInfo(JSONObject instructor) {
		...
		if (instructor.get("id").toString().equals("")) {
			...
			try {
				...
				SeUser entity = userDao.findById(instructor.get("id").toString());
				...
				entity.setScore(Integer.parseInt(instructor.get("score").toString()));

				/*
				 * if(student.get("studentcategory").toString().equals("1")||student
				 * .get("studentcategory").toString().equals("普通本科")){
				 * entity.setStudentcategory("1"); }else
				 * if(student.get("studentcategory"
				 * ).toString().equals("2")||student
				 * .get("studentcategory").toString().equals("普通专科")){
				 * entity.setStudentcategory("2"); }else{
